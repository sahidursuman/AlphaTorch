class NewCustomerForm
  ...
  def submit(params)
    ...
    if unique_primary_phone
      p 'UNIQUE PRIMARY PHONE NUMBER TRUE'
      if unique_email
        p 'UNIQUE EMAIL'
        if self.valid?
      ...
      p 'NOT A UNIQUE PRIMARY PHONE - RETURNING FALSE'
      return false
    end
  ...
  def update_attributes(params)
    ...
    if customer.changed?
      ...
      if unique_primary_phone
        p 'UNIQUE PRIMARY PHONE IS TRUE'
        if unique_email
          p 'UNIQUE EMAIL IS TRUE'
          if self.valid?
            ...
            p 'CUSTOMER CHANGED IS FALSE - INVALID'
            return false
          end
        else
          p 'NOT A UNIQUE EMAIL - RETURNING FALSE'
  ...
  def unique_email
    #if Customer.where("email = '#{customer.email}'#{' AND id = ' + customer.id.to_s unless customer.id.nil?}").exists? && !customer.email.empty?
    unless customer.email.blank?
      if (Customer.where(:email => customer.email).exists?) && (customer.email != customer.email_was)
